const Button = ({ children, variant = "primary", className = "", type = "button", ...props }) => {
  ...
  return (
    <button type={type} className={`px-4 py-2 rounded-md font-bold transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer text-[10px] uppercase ${variants[variant]} ${className}`} {...props}>
      {children}
    </button>
  );
};
